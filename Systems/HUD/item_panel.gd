...
func _ready():
	...
	print(materials_requirements)
	
func _process(delta):
	if int(item_count.text) <= 1:
		item_count.hide()
	else:
		item_count.show()
...
func _on_craft_button_pressed():
	if item_craftable:
		...
		backpack.add_inventory_item(selected_item, output)  # Assuming add_item is implemented in the backpack
